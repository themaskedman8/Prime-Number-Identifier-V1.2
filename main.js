...
function filter() {
    if (document.getElementById('input').value.length > 0) {
        if (document.getElementById('input').value >= 2) {
            ...
            setTimeout(backToWhite, 1000);
            console.error('ERROR_2: INVALID NUMBER');
            document.getElementById('error').innerHTML = 'ERROR_2: Must enter a natural number greater than 1.';
            return appendElem('ERROR_2');
        }
    } else {
        whiteToRedEffect();
        document.getElementById('buzzer').play();
...
function appendElem(n) {
    ...
    if (n == 'ERROR_1') {
        ...
        newElement.innerText = 'ERROR_2: INVALID NUMBER.';
        newElement.style = "color: red;";
    }
